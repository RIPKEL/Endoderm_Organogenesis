...
#>>>> My function
#--------------------------------
Myfilter = function(exp,gene,pearson.threshold=0.5,partner.threshold=5,expression.threshold=1,
         top.dispersion.interval=0.8,bottom.dispersion.interval=0,iteration=F){
  ...
  if(iteration){
    while (nrow(pearson)>length(gene.filtered)) {
      pearson=pearson[gene.filtered,gene.filtered]
      gene.filtered=rownames(pearson)[rowSums(pearson>pearson.threshold)>=partner.threshold]
    }
  }
...
MyHeatmap = function(
    ...
    type = c("log.row.zscore",
             ...
             "raw"),
    return.tree = "none",
    return.ordered.dendrogram = "none",
  ...
  if(hc.c.data.type == "raw"){
    ...
    hc.c.data <- as.matrix(log2(data + 0.1))
  ...
  if(hc.r.data.type == "raw"){
    ...
    hc.r.data <- as.matrix(log2(data + 0.1))
  ...
  if(graph){
    ...
    if(type == "log.row.zscore"){
      heatmap.data <- as.matrix(log2(data + 0.1))
  ...
  if (return.tree == "col" & return.ordered.dendrogram == "none"){
    return(c.hc)
  }else if (return.tree == "row" & return.ordered.dendrogram == "none"){
    return(r.hc)
  }
  if (return.ordered.dendrogram == "col" & return.tree == "none"){
    return(Colv)
  }else if (return.ordered.dendrogram == "row" & return.tree == "none"){
    return(Rowv)
  }
}
